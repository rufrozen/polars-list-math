...
Nested models are ordinary dataclasses with ordinary Python fields. The root
dataclass uses `@tp.model(schema=...)`, which validates the complete model tree
and caches its physical serialization plan in a `Builder` attached to the root
...
- `_compiler.py` validates dataclasses and compiles physical plans;
...
model-level `field`, dtype override, alias, or dynamic extras API. Flat storage
is configured by `Struct(flat=True)` or `ListStruct(flat=True)` in the schema.
